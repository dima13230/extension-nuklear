...
static struct nk_defold_handle {
	lua_State *L;
	int ref;
};

static struct nk_defold_font {
	struct nk_user_font font;
	struct nk_defold_handle handle;
};

static struct nk_defold_context {
    struct nk_context nkctx;
    struct nk_defold_font *fonts;
	int font_count;
	float *layout_ratios;
...
static void nuklear_Init(float width, float height)
{
    #if defined(IMGUI_IMPL_OPENGL_LOADER_GL3W)
    int r = gl3wInit();
    if (r != GL3W_OK) {
    ...
    &context->fonts[0].font = nk_font_atlas_add_default(&atlas, 14.0f, &cfg);
    image = nk_font_atlas_bake(&atlas, &w, &h, NK_FONT_ATLAS_RGBA32);
    device_upload_atlas(&device, image, w, h);

    nk_font_atlas_end(&atlas, nk_handle_id((int)device.font_tex), &device.tex_null);

    nk_init_default(&context->nktx, &context->fonts[0].font);
...
dmExtension::Result InitializeDefoldImGui(dmExtension::Params* params)
{
    ...
    nuklear_Init(displayWidth, displayHeight);
